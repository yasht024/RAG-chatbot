...
def render_response(internal_response: FactualResponse) -> dict:
    ...
    base = {
        ...
        } if internal_response.citation_url else None
    }
    
    if internal_response.status != TerminalState.FACTUAL_ANSWER:
        base["error"] = {
            "reason": internal_response.refusal_reason or "Request could not be processed due to insufficient evidence or ambiguity."
        }
    return base
