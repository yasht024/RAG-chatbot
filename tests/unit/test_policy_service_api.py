...
class TestPolicyServiceAPI(unittest.TestCase):
    ...
    def test_scan_query_sensitive(self):
        response = self.client.post(
            "/v1/policy/scan",
            json={"query": "My PAN is ABCDE1234F. What is the exit load?"},
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_classify_query_hindi_advisory(self):
        response = self.client.post(
            "/v1/policy/classify",
            json={"query": "क्या मुझे HDFC Mid Cap में निवेश करना चाहिए?"},
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
    ...
    def test_resolve_scheme_amc_disambiguation(self):
        response = self.client.post(
            "/v1/policy/resolve",
            json={"query": "What is the exit load for small cap fund?"},
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
    ...
    def test_render_refusal_hindi(self):
        response = self.client.post(
            "/v1/policy/refusal",
            json={"query_class": "ADVISORY", "query": "क्या मुझे निवेश करना चाहिए?"},
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
