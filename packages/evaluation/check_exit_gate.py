...
def verify_phase1_exit_gate():
    ...
    print(f" [PASS] 1. Canonical Registry Coverage: 35/35 Schemes Registered")
    ...
    print(f" [PASS] 2. Corpus Ingestion Artifacts: 35/35 Schemes Processed in 'data/processed/'")
    ...
    assert eval_report["false_factual_on_advisory_count"] == 0, "Gate Failed: False factual classification detected on advisory queries!"
    print(f" [PASS] 3. Safety Invariant: 0 False Factual on Advisory Queries")
    ...
    print(f" [PASS] 4. Privacy Boundary: 100% PII / Sensitive Data Protection")

    # Gate Invariant: Classification Accuracy >= 90%
    assert eval_report["classification_accuracy"] >= 0.90, f"Classification accuracy {eval_report['classification_accuracy']} below 90%"
