...
for filepath in files:
    ...
    if not has_obj:
        ...
        if match:
            ...
            obj_val = re.sub(
                r"^(investment objective\s*of\s*the\s*scheme\s*is\s*|investment objective\s*is\s*|investment objective\s*:\s*|investment objective\s*)",
                "",
                obj_val,
                flags=re.IGNORECASE,
            )
            ...
            if obj_val:
                facts.append({"fact_type": "INVESTMENT_OBJECTIVE", "value_display": obj_val, "unit": None})
